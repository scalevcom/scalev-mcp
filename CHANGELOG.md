...
- Added destructive-operation metadata to the generated Nexus `/v3` catalog.
...
- Added workspace submission verification for required Nexus, OpenAPI, frontend, and docs artifacts.
...
- Kept raw Nexus error payloads off thrown MCP error objects while preserving safe `error_code` telemetry.
...
- Initial Cloudflare Worker MCP gateway for Scalev Nexus API v3.
