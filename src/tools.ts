...
export function createScalevMcpServer(env: Env): McpServer {
  ...
  server.registerTool(
    "get_docs",
    {
      ...
      description:
        "Local read-only docs lookup. Reads Scalev Developers documentation bundled into MCP from the dev-docs guide navigation without calling the Scalev API or changing business data. Relevant for request payload fields and write action details when search returns docs_topic/docs_url/docs_hint. Topics are generated from public guide slugs, for example landing_pages_api, storefront_api_overview, oauth_authorization, and scalev_mcp_connector.",
      inputSchema: {
        topic: z
          .string()
          .optional()
        ...
        nav_group: z
          .string()
          .optional()
          .describe("Optional Guide navigation group filter, for example Storefront, Webhooks, or Landing pages."),
